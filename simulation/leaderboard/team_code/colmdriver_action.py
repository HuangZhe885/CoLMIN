...
class Reflect_Client:
	def __init__(self, url, api_key, api_base):
		if api_key!='':
			...
			self.model_name = self.client.models.list().data[0].id
		print("Reflect_Client:", self.model_name)
		self.nego_content = {}
	...
	# def generate_reflection(self, local_comm_content, all_candidates_info):
	# 	"""
	# 	Deep reflection that returns a concise, actionable JSON with:
	# 	{"mistake": "...", "new_rule": "...", "evidence": "..."}

	# 	Accepts:
	# 	- local_comm_content: list of per-round messages, each element is list of {'id':..., 'message':...}
	# 	- all_candidates_info: list of round dicts {'round': int, 'candidates': [...]}

	# 	Returns:
	# 	dict with keys 'mistake', 'new_rule', 'evidence'
	# 	"""
	# 	import json, re, textwrap

	# 	history_summary = ""
	# 	if isinstance(local_comm_content, dict):
	# 		items = sorted(local_comm_content.items())
	# 	elif isinstance(local_comm_content, list):
	# 		items = list(enumerate(local_comm_content))
	# 	else:
	# 		items = []

	# 	for round_idx, messages in items:
	# 		history_summary += f"Round {round_idx}:\n"
	# 		# messages might be list of {'id':..., 'message':...} OR list of tuples
	# 		if isinstance(messages, list):
	# 			for msg in messages:
	# 				if isinstance(msg, dict):
	# 					sender = msg.get('id', 'unknown')
	...
	# 					sender = 'unknown'
	# 					text = str(msg)
	# 				# collapse list message into compact text
	# 				if isinstance(text, list):
	# 					text = " | ".join([str(t) for t in text])
	# 				# sanitize
	# 				text = str(text).replace('{', '[').replace('}', ']').replace('%', '%%')
	# 				history_summary += f"  Vehicle {sender}: {text}\n"
	# 		else:
	# 			text = str(messages)
	# 			history_summary += f"  {text}\n"

	# 	candidates_summary = ""
	# 	for round_info in (all_candidates_info or []):
	# 		r = round_info.get('round', 'unknown')
	# 		candidates_summary += f"Round {r}:\n"
	# 		for idx, c in enumerate(round_info.get('candidates', [])):
	# 			if not isinstance(c, dict):
	# 				candidates_summary += f"  Candidate {idx}: [INVALID FORMAT]\n"
	# 				continue
	...
	# 			except Exception:
	# 				score_val = 0.0
	# 			sugg = str(c.get('suggestion', '')).replace('%', '%%')

	# 			candidates_summary += (
	# 				f"  Candidate {idx}: score={score_val:.3f}, suggestion='{sugg}'\n"
	# 			)
	# 	prompt = f'''
	# **Deep Reflection Task**
	# You are an analyst. Given the negotiation history and candidate plan summaries below,
	# identify *recurring mistakes* and propose **1 concrete, actionable rule** (max 1 sentence) 
	# that can be applied next round to reduce overconfidence, repetition, or deadlock.
	# Rules must be explicit and directly instruct vehicle behavior (examples:
	# "If no conflict detected for this vehicle in last 2 rounds, do not request yielding",
	# "Do not resend identical request more than 1 round; instead KEEP or SLOW"). 
	...
	# Evidence of overconfident outputs:from what perspective is this evidence identified? Please provide a thought process.
	# Provide as detailed evidence as possible to determine whether the model is overconfident or overly conservative, including your reasoning process — for example, due to historically low scores, repeated outputs, and so on.

	# Return EXACTLY a JSON object with keys:
	# {{"mistake":"...", "new_rule":"...", "evidence": "..."}}


	# Make "new_rule" short, specific and executable (no vague terms).
	# Make "evidence"  as detailed  as possible
	# '''
	...
	# 	except Exception:
	# 		# try cleaned-up transform then parse
	# 		try:
	# 			resp2 = resp.replace("\n", " ").replace("'", '"')
	# 			# extract a JSON-like substring
	# 			m = re.search(r'(\{.*\})', resp2)
	# 			if m:
	# 				reflection = json.loads(m.group(1))
	# 			else:
	# 				reflection = {"mistake": "", "new_rule": "", "evidence": ""}
	# 		except Exception:
	# 			reflection = {"mistake": "", "new_rule": "", "evidence": ""}

	# 	# normalize keys
	# 	for k in ["mistake", "new_rule", "evidence"]:
	# 		if k not in reflection:
	# 			reflection[k] = ""

	# 	# Ensure new_rule is short (truncate if necessary)
	# 	reflection['new_rule'] = str(reflection['new_rule']).strip()
	# 	if len(reflection['new_rule']) > 300:
	# 		reflection['new_rule'] = reflection['new_rule'][:300]
	# 	return reflection

	def generate_reflection(self, local_comm_content, all_candidates_info):
			"""
			Deep reflection that returns a concise, actionable JSON with:
			{"mistake": "...", "new_rule": "...", "trend_analysis": "..."}
			...
			Returns:
			dict with keys 'mistake', 'new_rule', 'trend_analysis'
			"""
			import json, re, textwrap
			...
			# 2) Build candidate summary safely
			candidates_summary = ""
			for round_info in (all_candidates_info or []):
				r = round_info.get('round', 'unknown')
				candidates_summary += f"Round {r}:\n"
			...
			try:
				...
				try:
					...
					if m:
						...
						reflection = {"mistake": "", "new_rule": "", "trend_analysis": ""}
				except Exception:
					reflection = {"mistake": "", "new_rule": "", "trend_analysis": ""}

			# normalize keys
			for k in ["mistake", "new_rule", "trend_analysis"]:
				if k not in reflection:
					reflection[k] = ""
...
class Comm_Client:
	...
	def comm_multi(self, comm_info_list, car_id, local_comm_content, suggestion, deep_memory_rule=None):
		"""
		生成每辆车的多条简短协商消息（4条，每条仅包含关键信息），并保证解析安全
		"""

		# 找到 ego 车辆信息
		info = next((c for c in comm_info_list if c['ego_id'] == car_id), None)
		if info is None:
			raise ValueError(f"Car ID {car_id} not found in comm_info_list")

		# 周围车辆信息字符串
		veh_string = ''
		for i in range(len(info['other_direction'])):
			veh_string += (
				f"  - Vehicle (ID: {list(info['other_direction'].keys())[i]}): "
				f"{round(list(info['other_distance'].values())[i], 1)}m at your {list(info['other_direction'].values())[i]}, "
				f"intention {list(info['other_intention'].values())[i]}\n"
			)

		# 对话历史
		previous_conv = ''
		for item in local_comm_content:
			previous_conv += f"- Vehicle {item['id']}: '{item['message']}'\n"
		previous_conv = previous_conv or 'None'

		# Deep memory / reflection
		deep_str = ''
		if deep_memory_rule:
			deep_str = "\n**Deep Reflection Rules:**\n"
			rules = deep_memory_rule.split('\n') if isinstance(deep_memory_rule, str) else deep_memory_rule
			for r in rules:
				deep_str += f"- {str(r).replace('{','{{').replace('}','}}')}\n"
		...
		prompt = f'''**Role**
	You are an autonomous driving assistant. Your goal is to negotiate with other cars to ensure safety and efficiency.
	...
	**Traffic Rules**
	1. Merging cars yield to straight cars.
	2. Left-turn cars yield to straight/right-turn cars.
	3. Vehicles being yielded to can go faster.
	4. Vehicles behind must slow down when the front car brakes.

	**Task**
	Generate 4 short and distinct negotiation messages for Vehicle {info['ego_id']}.
	Each message must:
	1. Represent different strategies: Aggressive (proceed first), Safe (yield), Compromise, Alternative or cooperative
	2. Focus only on the key action or request (no explanation or filler).

	**Conversation History**
	{previous_conv}{sug_str}
	**Deep Memory / Reflection Rules**
	{deep_str}
	...
	**Output Format**
	...
	No extra text, no explanations, no escape characters.
	Do not use contractions. Always write words in full. 
	'''

		# === 调用模型 ===
		message_str, comm_time = self.send_message(prompt, return_time=True)
		...
		def safe_json_parse(text):
			...
			try:
				...
				# 清除可能的包裹符号与非法转义
				text = re.sub(r'^[^[]*', '', text)  # 去掉前缀
				text = re.sub(r'[^]]*$', '', text)  # 去掉后缀
				text = text.replace('\\"', '"')
				try:
					return json.loads(text)
				except Exception:
					# 尝试按句号拆分
					return [s.strip().strip('"') for s in re.split(r'[.;!?]', text) if s.strip()][:4]

		message_list = safe_json_parse(message_str)

		# fallback 补足4条
		while len(message_list) < 4:
			message_list.append(f"Fallback message for Vehicle {car_id}")
		message_list = message_list[:4]

		return message_list, comm_time
...
You are Vehicle {info['ego_id']}, the message you want to negotiate with other cars is:'''
	...
	**Classification rules**
	...
	{"0": {"speed": "FASTER"}, "2":{"speed": "SLOWER"}},
	{"0": {"speed": "STOP"}, "2":{"speed": "KEEP"}},
	{"0": {"speed": "SLOWER"}, "2":{"speed": "SLOWER"}}
	]
	Input proposal:
	ppp
	...
	def sum_action_multi(self, comm_content):
		prompt = '''**Task**
		Given a conversation of multiple cars negotiating to reach consensus, generate 4 distinct candidate action plans. 
		Each action plan should classify every vehicle's speed change into [STOP, SLOWER, KEEP, FASTER].
		...
		**Additional rules**
		- If a car requests others to yield, it should go faster.
		- If a car yields to others, it should stop.

		**Safety and efficiency rules**
		- Always prioritize safety over speed: avoid collisions and maintain safe distance.
		- Maximize efficiency when possible: vehicles may accelerate or keep speed if it does not create conflicts.
		- If a car requests others to yield, it can cautiously increase speed, but only if safe.
		- If a car yields to others, it should slow down or stop to avoid conflict.
		- Avoid actions that could lead to conflicting maneuvers.

		**Diversity requirement**
		- Each candidate plan must be meaningfully different from the others.
		- "Meaningfully different" means at least one vehicle has a different action across plans.
		- Consider different conflict resolutions (e.g., A yields vs B yields).
		

		**Output requirements**
		- Output 4 distinct candidate action plans as a JSON list.
		- DO NOT include any extra text, explanations, or formatting outside the JSON.

		**Example output**
		[
		{"0": {"speed": "STOP"}, "2": {"speed": "SLOWER"}},
		{"0": {"speed": "KEEP"}, "2": {"speed": "FASTER"}},
		{"0": {"speed": "FASTER"}, "2": {"speed": "KEEP"}},
		{"0": {"speed": "SLOWER"}, "2": {"speed": "KEEP"}}
		]

		Input proposal:
		ppp
		Your task is to analyze the given conversations for each vehicle and output the classification in the specified format. 
		DO NOT output other content other than the required actions.'''
		...
		match = re.search(r'\[\s*{.*}\s*\]', clean, re.S)
		if match:
			clean = match.group(0)
		...
		try:
			action_candidates = json.loads(clean)
		...
		if not action_candidates:
			print("Using fallback candidate due to parse failure.")
			action_candidates = [deepcopy({str(item['id']): {"speed": "KEEP"} for item in comm_content})]

		return action_candidates, sum_action_time
...
Consensus score: int'''
	...
	def evaluator(self, comm_results, local_comm_content, waypoints, main_path, step):
		...
		if efficiency_score < 0.4:
			suggestion+='Efficiency issue: Some vehicles can go faster if it does not cause safety conflicts.\n'
...
class PnP_infer():
	def __init__(self, config=None, ego_vehicles_num=1, perception_model=None, planning_model=None, perception_dataloader=None, device=None) -> None:
		...
		#加入反思模型
		self.reflect_assistant = Reflect_Client(url=self.config['comm_server']['reflect_client'], api_key=self.config['comm_server']['api_key'], api_base=self.config['comm_server']['api_base'])
	...
	# def group_negotiation(self, group, comm_info_list, default_order, comm_results,
	# 					total_car_data_raw, total_rsu_data_raw, step, timestamp,
	# 					score_threshold=8.5, reflection_interval=2):
	# 	"""
	# 	Multi-agent negotiation function.

	# 	Args:
	# 		group: list of car_ids in the current negotiation group
	# 		comm_info_list: list of vehicle info dicts
	# 		default_order: list specifying negotiation order
	# 		comm_results: dict to store final negotiation results
	# 		total_car_data_raw, total_rsu_data_raw: environment info
	# 		step, timestamp: current simulation step and time
	# 		threshold: score threshold for early consensus
	# 	"""
	# 	self.local_max_round = 4
	# 	local_comm_content = []
	# 	suggestion = ''
	...
	# 	best_consensus_result = None
	# 	best_consensus_suggestion = ''
	...
	# 			# 每辆车生成多条候选消息
	# 			message, comm_time = self.comm_client.comm_multi(
	# 				comm_info_list, car_id, local_comm_content, suggestion_candidate, deep_memory_rule
	# 			)
	...
	# 		# ---------------- Critic 阶段：生成候选 ----------------
	# 		try:
	# 			action_candidates, sum_action_time = self.comm_client.sum_action_multi(
	# 				local_comm_content[-len(group):]
	# 			)
	# 			if not action_candidates:
	# 				action_candidates = [deepcopy(comm_results)]
	# 		except Exception as e:
	# 			print("sum_action_multi exception:", e)
	# 			action_candidates = [deepcopy(comm_results)]
	...
	# 		for idx, ac in enumerate(action_candidates):
	# 			print(f"  Candidate {idx}: {ac}")

	# 		candidates_info_this_round = []

	# 		# ---------------- 每个候选打分（带记忆修正） ----------------
	# 		for idx, candidate in enumerate(action_candidates):
	# 			candidate = {str(k): v for k, v in candidate.items()}

	# 			# 补全导航字段
	# 			for key in candidate.keys():
	# 				ego_id = int(key)
	# 				idx_list = [i for i, info in enumerate(comm_info_list) if info['ego_id'] == ego_id]
	# 				if not idx_list:
	# 					continue
	# 				idx0 = idx_list[0]
	# 				if 'nav' not in candidate[key] or candidate[key]['nav'] is None:
	# 					candidate[key]['nav'] = comm_info_list[idx0]['ego_intention']

	# 			# 生成预测路径
	# 			pred_waypoints = []
	# 			for _id in group:
	# 				key = str(_id)
	# 				idx_list = [i for i, info in enumerate(comm_info_list) if info['ego_id'] == _id]
	# 				if not idx_list:
	# 					continue
	# 				idx0 = idx_list[0]

	# 				if key not in candidate:
	# 					candidate[key] = {'speed': 'KEEP', 'nav': comm_info_list[idx0]['ego_intention']}

	# 				waypoints = self.get_action_from_list_inter(
	# 					_id, total_car_data_raw[_id], total_rsu_data_raw[_id],
	# 					step, timestamp, comm_info=candidate[key], nego=True
	# 				)
	# 				pred_waypoints.append(waypoints)

	# 			# 基础评分 + suggestion_candidate
	# 			score, suggestion_candidate, detailed_score = self.nego_assistant.evaluator(
	# 				candidate, local_comm_content, pred_waypoints, self.log_path, step
	# 			)

	# 			# ====== 历史风险修正 ======
	# 			history_risk = 0
	# 			adjusted_score = score * (1 - 0.2 * history_risk)

	# 			print(f"Candidate {idx} - base score: {score:.3f}, risk: {history_risk:.2f}, adjusted: {adjusted_score:.3f}, suggestion: {suggestion_candidate}")

	# 			# 更新轮次候选信息
	# 			candidates_info_this_round.append({
	...
	# 				best_result = deepcopy(candidate)
	# 				best_suggestion = suggestion_candidate

	# 			if suggestion_candidate.strip() == '' and adjusted_score > best_consensus_score:
	# 				best_consensus_score = adjusted_score
	# 				best_consensus_result = deepcopy(candidate)
	# 				best_consensus_suggestion = suggestion_candidate

	# 			# 提前终止条件
	# 			if suggestion_candidate.strip() == '' and adjusted_score >= score_threshold:
	# 				reached_consensus = True
	# 				comm_results = deepcopy(candidate)
	# 				suggestion = suggestion_candidate
	# 				print(f"\n✅ Round {k}: 达成共识且分数({adjusted_score:.3f})≥阈值({score_threshold})，提前结束。")
	# 				all_candidates_info.append({
	# 					'round': k,
	# 					'candidates': candidates_info_this_round,
	# 					'selected_candidate': deepcopy(candidate),
	# 					'selected_score': adjusted_score,
	# 					'selected_suggestion': suggestion_candidate
	# 				})
	# 				break
	# 			if k > 0 and k % reflection_interval == 0 and len(all_candidates_info) >= reflection_interval:
	# 			# if k == 1 and len(all_candidates_info) >= 2:
	# 				reflection = self.reflect_assistant.generate_reflection(local_comm_content, all_candidates_info)
	# 				print(f"Deep reflection at round {k}: {reflection}")
	# 				deep_memory_rule = reflection.get("new_rule", "").strip()
	# 				print("=== Debug: deep_memory_rule ===")
	# 				print("Type:", type(deep_memory_rule))
	# 				print("Content:", deep_memory_rule)

	# 		# if reached_consensus:
	# 		# 	break
	# 		if reached_consensus:
	# 			self.all_scores[k].append(adjusted_score)
	# 			break
			

	# 		# 保存轮次最佳信息
	# 		round_best = max(candidates_info_this_round, key=lambda x: x['score'])
	# 		all_candidates_info.append({
	# 			'round': k,
	# 			'candidates': candidates_info_this_round,
	# 			'selected_candidate': round_best['candidate'],
	# 			'selected_score': round_best['score'],
	# 			'selected_suggestion': round_best['suggestion']
	# 		})

	# 	# ---------------- 最终决策 ----------------
	# 	if reached_consensus:
	# 		final_result = comm_results
	# 		print(f"\n✅ 提前结束协商，最终结果为第 {k} 轮最佳方案。")
	# 	elif best_consensus_result is not None:
	# 		final_result = best_consensus_result
	# 		print(f"\n⚠️ 三轮后未提前结束，采用最高分共识方案（{best_consensus_score:.3f}）作为最终结果。")
	# 	else:
	# 		final_result = best_result
	# 		print(f"\n⚠️ 三轮后无共识，采用总体最高分方案（{best_score:.3f}）作为最终结果。")

	# 	# ---------------- 更新原有记录逻辑 ----------------
	# 	comm_results = final_result
	# 	for key_str, value in comm_results.items():
	# 		key_int = int(key_str)
	# 		if key_int not in self.round_comm_bank:
	# 			self.round_comm_bank[key_int] = []
	# 		self.round_comm_bank[key_int].append(value)

	# 	for id in group:
	# 		self.round_comm_complete_time[id].append(step * 0.05 + comm_time_all + sum_action_time_all)
	# 		self.next_comm_time[id] = step * 0.05 + comm_time_all + sum_action_time_all

	# 	for key_str, value in comm_results.items():
	# 		key_int = int(key_str)
	# 		self.prev_comm[key_int] = value
	# 		self.comm_bank[key_int] = value
	...
	def group_negotiation(self, group, comm_info_list, default_order, comm_results,
		...
		# all_scores 的合法轮次
		valid_round_ids = set(self.all_scores.keys())
		...
		for k in range(self.local_max_round):
			...
			# ---------------- 每个候选打分 ----------------
			for idx, candidate in enumerate(action_candidates):
				candidate = {str(k2): v for k2, v in candidate.items()}
				...
				print(f"Candidate {idx} - base score: {score:.3f}, risk: {history_risk:.2f}, "
					f"adjusted: {adjusted_score:.3f}, suggestion: {suggestion_candidate}")

				# 保存候选分数（★★★关键★★★）
				if k in valid_round_ids:
					self.all_scores[k].append(adjusted_score)
			...
			# ★★★ 保存轮次最终选中的分数（当没有提前结束）★★★
			if k in valid_round_ids:
				self.all_scores[k].append(round_best['score'])
	...
	def save_feedback_scores(self, save_path='/data1/huangzhe/projects/CoLMDriver/results/feedback_scores2.csv'):
		import csv
		...
		with open(save_path, 'a', newline='') as f:
			...
			# 写每一轮的分数（自动遍历所有轮次）
			for round_id, scores in self.all_scores.items():
				for s in scores:
					writer.writerow([round_id, s])
